...
// Root route: Serve index.html for browsers, raw script for Roblox
app.get('/', (req, res) => {
  if (isRoblox(req)) {
    res.type('text/plain');
    return res.send(`print("luau!")`);
  }
  // Always serve index.html for non-Roblox requests
...
app.get('*', (req, res) => {
  ...
  // Serve index.html for browsers on unmatched routes
  if (!isRoblox(req)) {
    return res.sendFile(path.join(__dirname, 'index.html'));
  }

  // Fallback for Roblox on unmatched routes
  res.type('text/plain');
